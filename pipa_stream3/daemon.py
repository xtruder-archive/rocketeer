...
def createDaemon():
   ...
   if (pid == 0):pid# The first child.
      ...
      try:
         ...
         pid = os.fork()fork# Fork a second child.
      except OSError, e:
         raise Exception, "%s [%d]" % (e.strerror, e.errno)

      if (pid == 0):pid# The second child.
         ...
         os._exit(0)_exit# Exit parent (the first child) of the second child.
   else:
      # exit() or _exit()?
      ...
      os._exit(0)_exit# Exit parent of the first child.
   ...
   for fd in range(0, maxfd):
      try:
         os.close(fd)
      except OSError:OSError# ERROR, fd wasn't open to begin with (ignored)
         pass
   ...
   os.open(REDIRECT_TO, os.O_RDWR)O_RDWR# standard input (0)

   # Duplicate standard input to standard output and standard error.
   os.dup2(0, 1)error# standard output (1)
   os.dup2(0, 2)output# standard error (2)
